...
void ITDB02::print(char *st, int x, int y, int deg)
{
	...
	if (x==CENTER)
		x=((disp_y_size+1)-(stl*(cfont.x_size-2)))/2;
	}

	for (i=0; i<stl; i++)
		if (deg==0)
			printChar(*st++, x + (i*(cfont.x_size-2)), y);
		else
			rotateChar(*st++, x, y, i, deg);
}

int ITDB02::printNumI(long num, int x, int y, boolean leftAlign)
{
  int width = 0;
  char buf[25];
  char st[27];
...
int ITDB02::getFontWidth()
{
	return cfont.x_size-2;
}

int ITDB02::getFontHeight()
{
	return cfont.y_size;
}
